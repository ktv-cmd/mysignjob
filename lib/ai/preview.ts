...
IF ILLUMINATED: the per-request description below names one or more lighting techniques — treat that combination as the complete and exact emission spec for this render, nothing more and nothing less. Render illumination the way a camera actually captures a lit sign at night: bright emissive surfaces with visible soft bloom/halation AT THEIR OWN SURFACE EDGES ONLY — this bloom is a property of the emissive material itself (like an overexposed light source in a photo), it never extends past the emissive element onto a separate non-lit surface a few pixels away, and it is NOT a substitute for a halo/wash technique the per-request description didn't ask for. Strong contrast against whatever stays dark. Note: many signs mount on a backer panel rather than bare wall — wherever this section says "the surface behind the sign," that means the panel's face when one is present, or the wall itself when there is none; never treat a distant, still-dark wall as satisfying a requirement about the panel directly behind the letters. Front-lit: subsurface scattering through the acrylic faces makes them glow brightly with visible bloom confined to the face itself; the surface directly behind the sign stays completely dark — zero glow, zero wash, zero halo or color aura bleeding onto it, even immediately at the letters' edges. Back-lit: a soft halo of light washes onto the surface directly behind the sign with visible inverse-square falloff (brightest near the sign, fading outward); the letter faces themselves stay fully opaque, matte, and dark — no light escapes the front. Side-mounted accent: a thin, crisp, bright LED rim-light traced along each letter's side return-plane edges only — it never by itself adds front-face glow or a wash on the surface behind the sign; combine it with front-lit and/or back-lit glow only when the per-request description names both together. Full/complete surround: front-lit face glow + back-lit wash on the surface behind the sign + FULLY GLOWING side return planes — the entire return-plane surface emits light at the SAME brightness as the front face (not a thin edge accent), so the lit face and lit side read as one continuous glowing volume from an angled view. ALL THREE techniques rendered at full brightness simultaneously — do not dim any one technique to accommodate the others.
